...
var despMesh =[0,0.85, 0.75, 0.5, 1.1]//
...
function addGUI() 
{
	...
	var guiC = gui.addFolder('Physics');
...
function main() {
	...
	loadGLTF('model/gltf/Holanda/Holanda.gltf', [0, 5, -10], [0.1, 0.1, 0.1]);
	

	//

	loadGLTF('model/gltf/Portugal/ScenePortugal.gltf', [0, 3, -5], [0.01, 0.01, 0.01]);
...
function initCannon(){
	...
	var boxShape3 = new CANNON.Box(new CANNON.Vec3(2.5,0.5,0.75));
	...
	var boxShape4 = new CANNON.Box(new CANNON.Vec3(2.5,1.15,0.75));
...
function updatePhysics(){
	...
	for(var i=1; i !== meshes.length; i++){
		var res;
		res=bodies[i].position.vsub(new CANNON.Vec3(0,despMesh[i],0));// 
